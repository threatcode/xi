...
public class DownloadReceiveCollegeList extends BaseAction{
	...
	//CSV file header
	private static final String FILE_HEADER = "EIIN,COLLEGE,DIST,MOBILE,RESULT PUBLISHED,NOT APPROVED,COLLEGE RECEIVED,ADMISSION CANCELLED";
	
	
	public String downloadReceiveCollegeListCSV(){
		...
		response.setHeader("Content-Disposition", "attachment; filename=\""+userDto.getBoardName()+"_ReceiveColleges_"+datatime+".csv\"");
...
/*			response.getOutputStream().write(header.toString().getBytes());			
			...
			for(int i=0;i<receivedCollegeList.size();i++){
				
				
				collegeDTO = receivedCollegeList.get(i);
				...
				line.append(collegeDTO.getTotalResult());
				line.append(COMMA_DELIMITER);
				
				line.append(collegeDTO.getNotApproved());
				line.append(COMMA_DELIMITER);
				
				line.append(collegeDTO.getCollegeRceived());
				line.append(COMMA_DELIMITER);
				
				line.append(collegeDTO.getAdmissionCancelled());
