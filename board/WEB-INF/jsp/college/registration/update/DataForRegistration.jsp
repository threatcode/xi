...
<%--  	}--%>
	...
	<div id="print-header1">Copyright message</div>
		...
		<div class="box-body table-responsive" id="printThis">
			...
			<table id="example1" class="table table-bordered table-striped">
				<thead>
					...
					<tr>
						<%--<th class="masked">SL</th>
						--%><th width="20px">eSIF</th>
						<th width="180px">Applicant's Name<br>Father's Name<br>Mother's Name</th>
						<th width="30px">Gender<br>DoB</th>
						<th width="25px">Section<br>Class Roll</th>
						<%--<th>Group<br>Shift<br>Version</th>
						--%><th>Subjects</th>
						<th>Opt<br>Sub </th>
						<th>Pass Year<br>Roll No<br>Board<br>RegNo</th>
						<th  width="50px" style="margin-top: 0px:margin-bottom: 0px:">Photo</th>
						<th class="masked1">Student<br>Signature</th>
				...
				<tbody>
					<c:forEach items="${requestScope.nonRegistered}" var="nonApprovedStudent" varStatus="loop">
						<tr>
							<%--<td class="masked"><div>${loop.index+1}
							</div></td>
							--%><td><div width="20px">${nonApprovedStudent.esif}</div></td>
							<td width="180px"><div>${nonApprovedStudent.applicantName1}</div></td>
							<td width="30px"><div>${nonApprovedStudent.gender1}</div></td>
							<td width="25px"><div>${nonApprovedStudent.sections1}</div></td>
							...
							<td width="50px" style="margin-top: 0px:margin-bottom: 0px:">
								<div>
								<img class="lazy" id="no-image${loop.index+1}" name="no-image${loop.index+1}" class="historyimage" src="${requestScope.PhotoPathTmp}${requestScope.eiinBoard}/${nonApprovedStudent.eiinCode}/19${nonApprovedStudent.boardID}_${nonApprovedStudent.eiinCode}${nonApprovedStudent.sscReg}.jpg" alt="No Photo" width="48" height="60" />
...
<script type="text/javascript">
	...
	/*
		$.ajax({
            type    : "POST",
            url     : "getDataForRegistrationUpdate",
            dataType: 'text',
            async   : false,
            data    : {
	...
	var d = new Date();
		...
		{
			changePhotoId = document.getElementById('no-image'+tmp.id);
			photoPath = document.getElementById('no-image'+tmp.id).src;
			tdRegCell = document.getElementById('td'+tmp.id);
			var res = tmp.name.split("###");
			ImageNumber = tmp.id;
			//console.log(res);
		    $.ajax({
		    	url: 'getIndividualDataUpdate',		
		 		type: 'POST',
		 		async: true,
				cache: false,
 ...
 (function() {
	...
	$('form').ajaxForm({
		...
		complete: function(xhr) {
			...
			{
				changePhotoId.src = photoPath +'?' + new Date().getTime();
