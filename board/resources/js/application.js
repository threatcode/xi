...
var arrYear = [2017,2016,2015,2014,2013,2012,2011,2010,2009,2008,2007,2006];
var arrBTEByear = [2017,2016,2015,2014,2013,2012,2011,2010,2009,2008,2007,2006];
var college_map = {}; 
...
function reloadYear(board_id){
 ...
 function validateSscInfo(){
	 ...
	 $("#personal_info_div").html("<img src='/board/resources/images/239.gif' />");
		 $.ajax({
			 	url: 'applicationInfoCheck.action',		
			 	type: 'POST',
...
function fetchCollegeList(search_type){
	...
	if(search_type=="by_thana"){
	 ...
	 $.ajax({
		 	url: 'getColleges',		
		 	type: 'POST',
		 	data: {eiin:$("#eiin").val(),thana_id:$("#thana_id").val(),helper_board_id:$("#helper_board_id").val(),college_search_type:search_type},
		 	success: function(data) {
		 		//console.log(data);
...
function submitMigrationNew(){
	...
	else if(!isGivenPriorityOkSS(submitTimePriorities))
	{
	...
	var data = $('#applicationForm').serializeArray();
	//console.log(data);
	$('.submitMigration').addClass("disabledMigrationSubmitButton");
	//$('.loadingSubmitImage').html('');
	$('.loadingSubmitImage').html('<img src=\'/board/resources/_images/loadingS.gif\' alt=\"Submitting Migration Request\" style=\"width:150px; height:150px; margin-left: 90px;\"/>');
	//$('.loadingSubmitImage').css('display','block');
	$(".loadingSubmitImage").show(100);
	scrollToBottom();
	$.ajax({
		url: 'submitMigrationNew.action',
		type: 'POST',
		data: data,
		timeout: 90000,
		success: function(data) {
			$('.submitMigration').removeClass("disabledMigrationSubmitButton");
			scrollToTop();
			if(data.status=='OK'){
				//alert(data.message);
				//setTimeout(function() { $("#right_content").html( data.message); }, 5000);
				 $("#right_content").html( data.message);
			}
			else if(data.status=='INVALID'){
				errorMessage=" <h2 class=\"mainHeader\">Migration Response</h2><div class='rounded-corner' style='height: 100px; width: 757px; text-align: center; background: #DDE8F0;font-size: 24px;color: #EC0F18;'><div class=\"successMessageDiv red\" id=\"finalResultDiv\"><center><div class=\"successResultMessage red\">Sorry, Your Migration Request is not successful.</div></center></div></div>";
				$("#right_content").html( errorMessage);
			}
			else{
				//alert(data.message);
				errorMessage=" <h2 class=\"mainHeader\">Migration Response</h2><div class='rounded-corner' style='height: 100px; width: 757px; text-align: center; background: #DDE8F0;font-size: 24px;color: #EC0F18;'><div class=\"successMessageDiv red\" id=\"finalResultDiv\"><center><div class=\"successResultMessage red\">Sorry, Your Migration Request is not successful.</div></center></div></div>";
